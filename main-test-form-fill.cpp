...
void FillFormField(FPDF_DOCUMENT doc, FPDF_FORMHANDLE form_handle, const std::wstring& field_name, const std::wstring& field_value) {
        ...
        for (int j = 0; j < annot_count; ++j) {
            ...
            if (subtype == FPDF_ANNOT_WIDGET) {
                ...
                if (name_length > 0) {
                    ...
                    if (field_name == std::wstring(name.begin(), name.end())) {
                        FPDF_WIDESTRING value = reinterpret_cast<const FPDF_WIDESTRING>(field_value.c_str());
...
int main() {
    ...
    /**** 填充表单字段内容 ****/
    FillFormField(doc, form_handle, L"Name", L"刘成强");
